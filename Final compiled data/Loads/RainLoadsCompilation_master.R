...
source(paste(Rlocal,"/Hydrovol/fxn_Hydrovol.R",sep=""))
source(paste(Rlocal,"/Hydrovol/fxn_WQcompos_cen.R",sep=""))

setwd(paste(Rlocal,"/MMSD_virus/Virus2/Rain",sep=""))
GMToffset <- 6*60*60

## Read in file with sample information
dfsamples.all <- read.delim(paste(Rlocal,"/MMSD_virus/Virus2/Final compiled data/Loads/PathFIB_concentrations.txt",sep=""))
...
for (i in 1:length(rain.site)){
  ...
  #Reformat date to POSIXct for rain file: this is already in GMT.
  ...
  dfsamples2$Hbpdate <- as.POSIXct(dfsamples2$start_date, format="%m/%d/%Y %H:%M",tz="CST6CDT")
  dfsamples2$Hepdate <- as.POSIXct(dfsamples2$end_date, format="%m/%d/%Y %H:%M",tz="CST6CDT")
  dfsamples2$Hbpdate <- as.POSIXct(format(as.POSIXct(dfsamples2$Hbpdate),tz="GMT",usetz=TRUE),format="%Y-%m-%d %H:%M",tz="GMT")
  dfsamples2$Hepdate <- as.POSIXct(format(as.POSIXct(dfsamples2$Hepdate),tz="GMT",usetz=TRUE),format="%Y-%m-%d %H:%M",tz="GMT")
  
  

  #Run scripts: 1. Hydrovol, 2. WQcompos, 3. RMeventsSamples
  dfsamples3 <- Hydrovol(dfQ=dfQ,Q="VALUE",time="pdate",df.dates=dfsamples2,bdate="Hbpdate",edate="Hepdate")
  
  #Define variables for compositing
  parms <- c("Adenovirus.C.D.F","Adenovirus.A","Enterovirus","Human_virus","Enterococci.CFU.100ml",
          "E..coli.CFU.100ml","BacSp.CN.100ml","Entero.CN.100ml","lachnocen","BacHumancen")
  #Define variables that indicate censored values
  cenparms <- c("lachno","BacHuman")
  cenvals <- c("BLD","BLD")
  
  #Transform variables to proper class for WQcompos_cen function
  for(j in 1:length(parms)) dfsamples3[,parms[j]] <- as.numeric(as.character(dfsamples3[,parms[j]]))
  for(j in 1:length(cenparms)) dfsamples3[,cenparms[j]] <- as.character(dfsamples3[,cenparms[j]])
  
  dfsamples4 <- WQcompos_cen(df.samples=dfsamples3,sampleID="USGS_ID",parms=parms,volume="event.vol",
                             cenparms=cenparms,cenvals=cenvals)

  dfsamples5 <- RMeventsSamples(df=dfRain,ieHr=12,rain="rain",time="pdate",dfsamples=dfsamples4,bdate="Hbpdate",edate="Hepdate")
  
  #WRITE FUNCTION TO COMPUTE LOADS FOR MULTIPLE PARAMETERS RATHER THAN JUST ONE.
  cf2liters <- 28.316846592
  parmsLoads <- paste(parms,"_load",sep="")
  
  #compute loads: result is millions of genomic copies (QPCR) or millions of CFUs (culture FIB)
  dfsamples5[,parmsLoads] <- dfsamples5[,parms]*dfsamples5$event.vol*cf2liters/1000000

  #NEED TO DETERMINE WHETHER IT IS WORTHWHILE TO CALCULATE AVERAGE DAILY LOAD FOR EVENT
  #THIS WOULD POSSIBLE BE VALUABLE FOR INCLUSION OF BASEFLOW IN THE OVERALL ANALYSIS
#  dfsamples5$Daily_Human_virus <- dfsamples5$Load_Human_virus/dfsamples5$duration*24
    
  
  if(i==1) {dfsamples.all2 <- dfsamples5
  }else dfsamples.all2 <- rbind(dfsamples.all2,dfsamples5)
...
filenm <- paste(Rlocal,"/MMSD_virus/Virus2/Final compiled data/Loads/PathLoads","_",format(Sys.time(), "%b%d%Y_%H%M"),".txt",sep="")
write.table(dfsamples.all2,filenm,sep="\t",row.names=F)
