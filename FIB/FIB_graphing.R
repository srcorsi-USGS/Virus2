...
FIB <- read.delim(paste(Rlocal,"/MMSD_virus/FIB.txt",sep=""))
names(FIB) <- c("FT","Site.Code","Abb","Sample.Date","Event","Entero.Culture","EC.Culture",
...
levels(FIB$Event)
FIB$Abb <- factor(FIB$Abb,c("LD","MF","UW","HW","MW","MC"))
FIB$Event <- factor(FIB$Event,c("Baseflow","Rain","CSO"))

## Boxplot of Human Bacteroides  ##
options(scipen=10)
bx <- boxplot(BacHuman~Event+Abb,data=FIB,xaxt="n", main = "",
